...
export async function registerRoutes(app: Express): Promise<Server> {
  ...
  // Sample Data API
  app.post("/api/stores/:storeId/sample-data", async (req, res) => {
    try {
      const { storeId } = req.params;
      
      // Create sample store if it doesn't exist
      let store = await storage.getStoreByShopifyId(storeId);
      if (!store) {
      ...
      // Check for existing sample data to prevent duplicates
      const existingProducts = await storage.getProductsByStore(store.id);
      const existingMilestones = await storage.getMilestonesByStore(store.id);
      const existingCampaigns = await storage.getDiscountCampaignsByStore(store.id);
      const existingPromotions = await storage.getSeasonalPromotionsByStore(store.id);

      if (existingProducts.length > 0 || existingMilestones.length > 0 || 
          existingCampaigns.length > 0 || existingPromotions.length > 0) {
        return res.json({
          success: false,
          ...
          data: {
            ...
            campaigns: existingCampaigns.length,
      ...
      const sampleProducts = [
        { title: "Luxury Face Serum", handle: "luxury-face-serum", price: "2500.00", imageUrl: "/images/serum.jpg", isEligibleForRewards: true },
        { title: "Anti-Aging Moisturizer", handle: "anti-aging-moisturizer", price: "1800.00", imageUrl: "/images/moisturizer.jpg", isEligibleForRewards: true },
        { title: "Vitamin C Eye Cream", handle: "vitamin-c-eye-cream", price: "1200.00", imageUrl: "/images/eye-cream.jpg", isEligibleForRewards: true },
        { title: "Hydrating Face Mask", handle: "hydrating-face-mask", price: "800.00", imageUrl: "/images/face-mask.jpg", isEligibleForRewards: true },
        { title: "Organic Cleanser", handle: "organic-cleanser", price: "950.00", imageUrl: "/images/cleanser.jpg", isEligibleForRewards: true },
        { title: "Premium Lipstick Set", handle: "premium-lipstick-set", price: "1500.00", imageUrl: "/images/lipstick.jpg", isEligibleForRewards: true },
        { title: "Foundation Bundle", handle: "foundation-bundle", price: "3200.00", imageUrl: "/images/foundation.jpg", isBundle: true, isEligibleForRewards: false },
        { title: "Skincare Starter Kit", handle: "skincare-starter-kit", price: "2800.00", imageUrl: "/images/starter-kit.jpg", isBundle: true, isEligibleForRewards: false },
        { title: "Glow Serum", handle: "glow-serum", price: "2200.00", imageUrl: "/images/glow-serum.jpg", isEligibleForRewards: true },
        { title: "Night Repair Cream", handle: "night-repair-cream", price: "2900.00", imageUrl: "/images/night-cream.jpg", isEligibleForRewards: true }
      ];

      const createdProducts = [];
      for (const product of sampleProducts) {
        const productData = insertProductSchema.parse({
          shopifyProductId: `shopify-${product.handle}`,
          storeId: store.id,
          ...product
        });
      ...
      // Get eligible product IDs for milestones
      const eligibleProductIds = createdProducts
        .filter(p => p.isEligibleForRewards)
        .map(p => p.id);

      // Sample milestones with different reward types
      const sampleMilestones = [
        {
          name: "Free Delivery Milestone",
          description: "Get free delivery on orders above PKR 2,500",
        ...
        {
          name: "Free Product Selection",
          description: "Choose a free product with orders above PKR 5,000",
          thresholdAmount: "5000.00",
          currency: "PKR",
          rewardType: "free_products",
          freeProductCount: 1,
          eligibleProducts: eligibleProductIds.slice(0, 5),
        ...
        },
        {
          ...
          description: "15% discount for VIP customers on orders above PKR 3,000",
        ...
        },
        {
          ...
          description: "PKR 500 off for new customers on orders above PKR 4,000",
          thresholdAmount: "4000.00",
          currency: "PKR",
          rewardType: "discount",
        ...
        {
          name: "Premium Free Products",
          description: "Choose 2 premium products with orders above PKR 8,000",
          thresholdAmount: "8000.00",
          currency: "PKR",
          rewardType: "free_products",
          freeProductCount: 2,
          eligibleProducts: eligibleProductIds.slice(2, 8),
          enableProductSelection: true,
          status: "active",
          priority: 5,
          displayOrder: 5,
      ...
      const sampleCampaigns = [
        ...
        },
        {
          ...
          description: "Tiered discounts: 10% off PKR 2000+, 15% off PKR 4000+, 20% off PKR 6000+",
          type: "tiered",
          status: "draft",
          priority: 5,
          usageLimit: 800,
          minimumOrderValue: "2000.00",
          maximumDiscountAmount: "3000.00",
          stackable: false,
          customerSegment: "all"
        }
      ];
      ...
      for (const campaign of sampleCampaigns) {
        const campaignData = insertDiscountCampaignSchema.parse({
          storeId: store.id,
          ...campaign
        });
        const createdCampaign = await storage.createDiscountCampaign(campaignData);
        createdCampaigns.push(createdCampaign);
      }

      // Sample seasonal promotions
      const samplePromotions = [
        {
          name: "Eid Beauty Collection",
          theme: "eid",
          bannerText: "Celebrate Eid with 25% off all premium beauty products! ✨",
          bannerColor: "#16A085",
          textColor: "#FFFFFF",
          autoActivate: false,
          activationDate: new Date(),
          deactivationDate: new Date(Date.now() + 7 * 24 * 60 * 60 * 1000), // 7 days from now
          isActive: true
        },
        {
          name: "Winter Skincare Special",
          theme: "winter",
          bannerText: "Winter skincare essentials - Up to 30% off moisturizers and serums! ❄️",
          bannerColor: "#3498DB",
          textColor: "#FFFFFF",
          autoActivate: true,
          activationDate: new Date(Date.now() + 10 * 24 * 60 * 60 * 1000), // 10 days from now
          deactivationDate: new Date(Date.now() + 90 * 24 * 60 * 60 * 1000), // 90 days from now
          isActive: false
        },
        {
          name: "Valentine's Day Romance",
          theme: "valentine",
          bannerText: "Love yourself this Valentine's - Special gift sets and romantic colors! 💕",
          bannerColor: "#E74C3C",
          textColor: "#FFFFFF",
          autoActivate: false,
          activationDate: new Date(Date.now() + 30 * 24 * 60 * 60 * 1000), // 30 days from now
          deactivationDate: new Date(Date.now() + 37 * 24 * 60 * 60 * 1000), // 37 days from now
          isActive: false
        },
        {
          name: "Summer Glow Campaign",
          theme: "summer",
          bannerText: "Get your summer glow on! Lightweight formulas and sun protection ☀️",
          bannerColor: "#F39C12",
          textColor: "#FFFFFF",
          autoActivate: true,
          activationDate: new Date(Date.now() + 60 * 24 * 60 * 60 * 1000), // 60 days from now
          deactivationDate: new Date(Date.now() + 150 * 24 * 60 * 60 * 1000), // 150 days from now
          isActive: false
        }
      ];
      ...
      for (const promotion of samplePromotions) {
        const promotionData = insertSeasonalPromotionSchema.parse({
          storeId: store.id,
          ...promotion
        });
        const createdPromotion = await storage.createSeasonalPromotion(promotionData);
